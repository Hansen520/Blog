...
/*
  方法1: 将数字转化为字符换，使用正则匹配其中的1的数量
  方法2: 将数字转换为字符串，再转换成数组，通过filter方法过滤出为1的值
*/

function findOne(n) {
  return Array.from({ length: n }, (v, i) => i + 1).reduce((pre, cur) => {
    // 转为数组并获取长度，没有则为+0
    // const curCount = String(cur).match(/1/g)?.length || 0;
    const curCount = String(cur)
      .split('')
      .filter((i) => i === '1').length;
    return pre + curCount;
  }, 0);
...
/*
  方法3: 以数学的方式寻找其中的规律(我看了一遍，有点困难，了解就好)
  如 222
  个位产生1 -> 1,      11,21,31,...,221 => 23 // 个位的1, 22个十
  十位产生1 -> 10-19   110-119 210-219 => 30// 相当于每个段落走10次，2个百
  百位产生1 -> 100-199               => 100 // 相当于每个段落走100次， 0个千
  23  +  30 + 100 = 153
